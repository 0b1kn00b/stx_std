...
class Types{
  static public inline function definition(v:Dynamic):Class<Dynamic>{
  ...
  static public function ancestors<A>(type:Class<A>,?until:Class<Dynamic>):Array<Class<Dynamic>>{
    ...
    if(until!=null){
      o = T.whilst(
        function(x:Class<Dynamic>){
          return x.typeof().name() != until.typeof().name();
        }
      )(T.toArray).reduceBy(o,[]);
      o.push(until);
    }
    return o;
  }
  ...
  static public function construct<A>(type:Class<A>,?args:Array<Dynamic>):Null<A>{
    ...
    return try{
      ...
      throw new Error(500,Std.string(e));
    }
  }
  @doc("")
  @:note('#0b1kn00b: could generalise this for enums.')
  static public function build<A>(name:String,?args:Array<Dynamic>):Maybe<A>{
    return (classify(name):Posit).flatMap(
      construct.bind(_,args).then(Maybe.pure)
    );
  }
  ...
  static public inline function descended(type : Class<Dynamic>, sup : Class<Dynamic>):Bool{
   ...
   static public inline function of<T>(type : Class<T>, value : Dynamic) : Null<T>{
      return try{
        ...
        throw new Error(500,'could not cast value to ${name(type)}');
      }
  }
